...
class TadashiEnv(gym.Env):
    ...
    def reset(self, seed=None):
        ...
        if self.baseline_fitness is None:
            self.baseline_fitness = evaluateList(
                self.app_factory, [], self.n_trials, self.timeout
            )
    ...
    def _get_obs(self):
        ...
        for i, tr in enumerate(self.transformation_list[-10:]):
            if i < 10:
                ...
                obs[2 + i * 2] = (
                    list(TrEnum).index(tr[1]) if isinstance(tr[1], TrEnum) else 0
                )
...
class ReinforcementLearning:
    ...
    def fit(self):
        ...
        for episode in range(self.max_episodes):
            ...
            if len(final_list) > 0:
                final_fitness = evaluateList(
                    self.app_factory, final_list, self.n_trials
                )
        ...
        print(
            f"Best transformation list ({len(self.best_transformation_list)} transformations):"
        )
        ...
        if len(self.best_transformation_list) > 0:
            ...
            isOutputMatching(
                arrays_original, self.app_factory, self.best_transformation_list
            )

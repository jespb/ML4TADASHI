...
class BeamSearch:
    ...
    def __init__(self, args):
        ...
        self.app_factory = Polybench(
            args.benchmark, base=base, compiler_options=[dataset, oflag]
        )
    ...
    def fit(self):
        ...
        op_list = []
        baseline_time = evaluateList(
            app_factory, op_list, timeout=timeout, n_trials=n_trials
        )
        ...
        for depth in range(max_depth):
            ...
            for score, path in beams:
                for action in getNextOperations(
                    app_factory, path, beam_width=beam_width
                ):
            ...
            if n_threads == 1:
                for path in new_paths:
                    new_score = evaluateList(
                        app_factory, path, timeout=timeout, n_trials=n_trials
                    )
            ...
            for s, p in beams[:]:
                print(
                    "Depth: %2d, Score: %.6f, Speed up: %.4f, Path: %s"
                    % (depth + 1, s, baseline_time / s, str(p))
                )
